...
const PlantaTable = ({ plantas, onEdit, onDelete, onAddNew }) => {
  const [filterText, setFilterText] = useState('');
  const [sortAscending, setSortAscending] = useState(true);

  const filteredPlantas = plantas.filter(planta => 
    planta.TX_DESCRICAO.toLowerCase().includes(filterText.toLowerCase())
  ).sort((a, b) => {
    if (sortAscending) {
        ...
        <TableBody>
          {filteredPlantas.map((planta) => (
            <TableRow key={planta.ID}>
              <TableCell>{planta.ID.toString().padStart(6, '0')}</TableCell>
              <TableCell>{planta.TX_DESCRICAO}</TableCell>
              <TableCell className="text-right">
                <Button variant="ghost" size="icon" onClick={() => onEdit(planta)}>
                  <Edit className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" onClick={() => onDelete(planta.ID)}>
